...
enum
{
    CORSAIR_DRAM_REG_COMMAND                = 0x20,     /* Command write register               */
    CORSAIR_DRAM_REG_DIRECT_COMMAND         = 0x31,     /* Direct colors write register         */
    CORSAIR_DRAM_REG_DIRECT_COMMAND_2       = 0x32,     /* Direct colors write register 2       */
    CORSAIR_DRAM_REG_IDENTIFICATION_DATA    = 0x40,     /* Read identification data register    */
};
...
{
public:
    CorsairDRAMController(i2c_smbus_interface *bus, corsair_dev_id dev, unsigned int leds_count, std::string dev_name);
    ...
    void            SetColorsDirect(RGBColor* colors);
